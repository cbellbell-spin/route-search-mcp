...
import { z } from "zod";
import { RideWithGPSApi, RideWithGPSApiError } from "./api.js";
// Constants
...
// ─────────────────────────────────────────────────────────────────────────────
server.registerTool("search_routes_near", {
    ...
    try {
        ...
        if (latitude !== undefined && longitude !== undefined && radius_km !== undefined) {
            ...
            routes = routes.filter(r => {
                if (r.first_lat === undefined || r.first_lng === undefined)
                    return false;
...
// ─────────────────────────────────────────────────────────────────────────────
server.registerTool("get_route_elevation_profile", {
...
server.registerTool("geocode_location", {
    ...
    try {
        ...
        const output = results.map((r, i) => `${i + 1}. ${r.display_name}\n   Lat: ${r.lat}, Lon: ${r.lon} (${r.type})`).join("\n\n");
        return {
            content: [{
                    type: "text",
